...
### 新機能（v2.2）
- **拠点管理番号ドリルダウン機能**: mainシートの一覧から拠点管理番号をクリックして詳細シートを表示
- **戻るボタン**: 拠点詳細シートから一覧へ簡単に戻れるナビゲーション
...
- **拠点管理番号シート**: 各拠点の詳細データ用シート（例：`osaka_desktop`、`kobe`、`himeji`など）
- **列構成**: システムが期待する列構成に合わせてください

**機能の流れ:**
1. 拠点を選択するとmainシートの一覧が表示されます
2. 一覧内の拠点管理番号をクリックすると、その拠点管理番号と同名のシートが表示されます
3. 戻るボタンで一覧に戻ることができます

例）拠点管理番号が `osaka_desktop` の場合：
- mainシートの一覧で `osaka_desktop` をクリック
- スプレッドシート内の `osaka_desktop` シートの詳細データが表示される
...
### 高度な機能
- **列の並び替え**: テーブルヘッダーをドラッグ&ドロップ
- **ステータス更新**: 貸出中機器のステータスをドロップダウンで変更
- **フィルタリング**: 「一覧」「貸出中」でデータを絞り込み
- **拠点ドリルダウン**: 一覧から拠点管理番号をクリックして詳細データを表示
